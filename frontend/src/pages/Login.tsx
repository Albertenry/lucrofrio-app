...
const Login = () => {
  const [email, setEmail] = useState('admin@lucrofrio.com');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/auth/login', {
        ...
        body: JSON.stringify({ email, senha: password }), // Note que aqui usamos "senha", não "password"
      });
      ...
      } else if (data.user.funcao === 'supervisor') {
        navigate('/supervisor/dashboard');
      } else if (data.user.funcao === 'tecnico') {
      ...
      setError(err.response?.data?.msg || 'Credenciais inválidas. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <form onSubmit={handleLogin}>
          ...
          <div className="mb-6">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full bg-primary hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:shadow-outline transition duration-300 ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
